...
function newCaseDialog(invoiceId) {
  ...
  const entityOptions = value => state.entities.map(en => `<option value="${en.id}" ${en.id === value ? 'selected' : ''}>${e(en.name)} · ${en.currency}</option>`).join('');
